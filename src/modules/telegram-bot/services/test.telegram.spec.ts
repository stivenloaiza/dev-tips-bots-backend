...
describe('TelegramBotService', () => {
  ...
  describe('getTip', () => {
    ...
    it('Should call formatTipMessage with the correct param', async () => {
      const formatTipMessageSpy = jest
        .spyOn<any, string>(service as any, 'formatTipMessage')
    ...
    it('Should call axios.post with the correct parameters', async () => {
      jest
        .spyOn(service as any, 'formatTipMessage')
        .mockReturnValue('formatted message');
      jest.spyOn(service, 'saveTipToDatabase').mockResolvedValue({} as any);
    ...
    it('Should call saveTipToDatabase with the correct param', async () => {
      jest
        .spyOn(service as any, 'formatTipMessage')
        .mockReturnValue('formatted message');
      const saveTipToDatabaseSpy = jest
        .spyOn(service, 'saveTipToDatabase')
    ...
    it('Should throw error when the axios.post fails', async () => {
      jest
        .spyOn(service, 'formatTipMessage')
        .mockReturnValue('formatted message');
      (axios.post as jest.Mock).mockResolvedValue({} as any);
      jest
        .spyOn(service, 'saveTipToDatabase')
        .mockRejectedValue(new Error('Axios fails, Database save fail'));

      await expect(service.getTip(tipDto)).rejects.toThrow(
      ...
      jest
        ...
        .mockRejectedValue(
          new InternalServerErrorException(
            'Internal server exception, Database save fail',
          ),
        );
